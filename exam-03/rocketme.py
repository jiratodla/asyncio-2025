...
async def main():
    ...
    tasks = [asyncio.create_task(fire_rocket(name, t0)) for name in names]

    # รอให้ทุก task เสร็จและเก็บผลลัพธ์
    results = await asyncio.gather(*tasks)

    # เรียงผลลัพธ์ตามเวลาถึงเป้าหมาย (time_to_target)
    results.sort(key=lambda r: r["time_to_target"])
    ...
    # แสดงเวลารวม ตั้งแต่ยิงลูกแรกถึงลูกสุดท้ายถึงเป้าหมาย
